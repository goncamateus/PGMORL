...
if record_video:
    env = wrappers.Monitor(
        env, os.path.join(save_path, "videos", record_video_filename), force=True, video_callable=lambda episode_id: True
    )
...
for _ in range(10):
    ...
    while not done:
        ...
        _, action, _, _ = policy.act(
            torch.Tensor(obs).unsqueeze(0), None, None, deterministic=False
        )
    ...
    print("iter = ", iter)
    print("objective = ", obj)
    print("Final Position = ", info["Final_Position"])
